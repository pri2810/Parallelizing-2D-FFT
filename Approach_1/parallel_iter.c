...
Complex** convert(PPMImageGS *im, int p)
{
  int rows = im->x;
  int cols = im->y;
  int i,j,idx;
  ...
  # pragma omp parallel \
    shared ( arr,im, rows,cols ) \
    private ( j,idx ) num_threads(p)
...
/*------------------------------look up table-----------------------------------------------*/
void twiddle( int n, double w[], int p )
{
  double arg;
  double aw;
  int i;
...
/*-------------------------------------FFT--------------------------------*/
void FFT(double *x, double *y, int n, double w[], int p)
{
   ...
   for(k=0;k<m;k++)
   {
	...
	term_i = n/mi;
	#pragma omp parallel \
	shared(x,y,mj,mi,n,term_i) \
	private(count2,j2,j,t1,t2,u1,u2) num_threads(p)
...
/*-----------------------------------2D FFT------------------------------*/
void FFT_2D(Complex **comp_in,int rows, int cols, double *w, int p)
{
...
Complex ** transpose(int N,Complex **comp_in,int p){
  ...
  {
	#pragma omp parallel \
	  shared(comp_in,arr,blockrow) \
	  private(i,j) num_threads(p)
	  #pragma omp for nowait
...
PPMImageGS * convert_comp_img(Complex **comp_in,int rows,int cols, int p)
{
  int i,j;
  ...
  # pragma omp parallel \
    shared (im2, rows,cols ) \
    private ( j,idx,temp ) num_threads(p)
...
int main(int argc, char* argv[])
{
  ...
  comp_in = convert(image, p);

  twiddle(rows,w,p); 

  FFT_2D(comp_in, rows, cols,w,p);
	
  comp_in = transpose(rows,comp_in,p);
   
  FFT_2D(comp_in,rows,cols,w,p);
     
  comp_in = transpose(cols,comp_in,p);

  transformed_img = convert_comp_img(comp_in,rows,cols,p);
  ...
  strcat(out_file,"fft_parallel.ppm");
